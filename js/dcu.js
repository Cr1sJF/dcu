  ...
  for (var key in keywords) {
    if (data.sterror.indexOf(key) != -1) {
      response[keywords[key]]
        ? (response[keywords[key]] += "\n" + data.sterror)
        : (response[keywords[key]] = data.sterror);
    }
  }
  response.INFO
    ? (response.INFO += "\n" + data.data)
...
function findEnvironment(folder) {
  ...
  if (!folder) {
    ...
    if (editor) {
      ...
      for (let i in CONST.VALID_PATHS) {
        ...
        if (path.indexOf(componentType) != -1) {
          ...
          if (
            componentType === CONST.COMPONENTS.WIDGET ||
            componentType === CONST.COMPONENTS.ELEM ||
            componentType === CONST.COMPONENTS.THEME
          ) {
            env.componentName = path[path.indexOf(componentType) + 1];
            env.componentPath = path
              .slice(0, path.indexOf(env.fileType) + 2)
              .join("/");
          } else {
            env.componentName = path[path.indexOf(componentType)];
            env.componentPath = path
              .slice(0, path.indexOf(env.fileType) + 1)
              .join("/");
          }
          break;
        }
    ...
    let pathInfo = this.buildPath(folder);
    configPath = pathInfo.basePath + "/.ccc/config.json";
    env.basePath = pathInfo.basePath;
    env.fileType = pathInfo.componentType;
    env.componentName = pathInfo.componentName;
    env.componentPath = pathInfo.componentPath;
  }
  ...
  if (
    getConfig(CONST.CONFIG.DEV, CONST.CONFIG.PROPS.ENVIROMENT_URL) &&
    new URL(getConfig(CONST.CONFIG.DEV, CONST.CONFIG.PROPS.ENVIROMENT_URL))
      .origin === env.envUrl.origin
  ) {
    ...
    getConfig(CONST.CONFIG.TEST, CONST.CONFIG.PROPS.ENVIROMENT_URL) &&
    new URL(getConfig(CONST.CONFIG.DEV, CONST.CONFIG.PROPS.ENVIROMENT_URL))
      .origin === env.envUrl.origin
  ) {
    ...
    getConfig(CONST.CONFIG.STAGE, CONST.CONFIG.PROPS.ENVIROMENT_URL) &&
    new URL(getConfig(CONST.CONFIG.DEV, CONST.CONFIG.PROPS.ENVIROMENT_URL))
      .origin === env.envUrl.origin
  ) {
    ...
    getConfig(CONST.CONFIG.PROD, CONST.CONFIG.PROPS.ENVIROMENT_URL) &&
    new URL(getConfig(CONST.CONFIG.DEV, CONST.CONFIG.PROPS.ENVIROMENT_URL))
      .origin === env.envUrl.origin
  ) {
    ...
    this.console.error("No hay configuración de sitios cargada");
  }
  return null;
}
...
module.exports = {
  ...
  buildPath: (pathString) => {
    if (typeof pathString != "string") {
      pathString = pathString.path ? pathString.path : pathString.fsPath;
    }
    ...
    let component;
    let basePath, componentPath, componentName;
    for (var i = 0; i < pathArray.length; i++) {
      if (CONST.VALID_PATHS.indexOf(pathArray[i]) != -1) {
        component = pathArray[i];
        if (CONST.UNNESTEABLE_COMPONENTS.includes(component)) {
          componentName = component;
        } else {
          componentName = pathArray[i + 1] ? pathArray[i + 1] : pathArray[i];
        }
        break;
      }
    }
    ...
    if (component) {
      basePath = pathArray.slice(0, pathArray.indexOf(component));
      componentPath = pathArray.slice(pathArray.indexOf(component));
      return {
  ...
  error: async function (options) {
    ...
    if (typeof options === "string") {
      ...
      model.msg = CONST.MSGS[options.msg]
        ? CONST.MSGS[options.msg]
        : options.msg;
  ...
  initializeFileTracking: () => {
    if (getConfig(CONST.CONFIG.GENERAL, CONST.CONFIG.PROPS.FILE_TRACKING)) {
      ...
      fileTracking.appendLine(
        `********* SESIÓN INICIADA A LAS: ${new Date().getHours()}:${new Date().getMinutes()}hs. **********`
      );
    }
  },
    ...
    cmd.run(
      ...
      function (error, data, stderr) {
        ...
        if (taskInfo.STATUS == CONST.STATUS.SUCCESS) {
          ...
          if (fileTracking) {
            fileTracking.appendLine(
              replace(item.MSGS.TRACKING, replacementData)
            );
            fileTracking.appendLine("");
          }
          ...
          if (showNotif == CONST.SI || showNotif == CONST.SOLO_ERRORES) {
            API.error({
              msg: item.MSGS.ERROR,
              replace: replacementData,
            });
        ...
        log({
          section: `END ${item.itemBar.tooltip}`,
          title: null,
          detail: null,
        });
      }
  ...
  validateVersion: function (STORAGE) {
    ...
    let lastVersion;
    let lastRelease =
      CONST.VERSIONS.RELEASES[CONST.VERSIONS.RELEASES.length - 1];

    if (!localVersion || utils.compare(localVersion, currentVersion) == 0)
      return;

    if (
      !STORAGE.get(CONST.STORAGE.VERSION) ||
      utils.compare(localVersion, currentVersion) == -1
    ) {
    ...
    if (lastVersion.TYPE === "R") {
      if (
        dcu.getConfig(
          CONST.CONFIG.GENERAL,
          CONST.CONFIG.PROPS.NOTIFY_UPDATES
        ) === CONST.PREGUNTAR
      ) {
        dcu.info({
        ...
        vscode.window.showInformationMessage(
          `Hey! Mirá las novedades de DCU UTILS`
        );
        setTimeout(() => {
          vscode.env.openExternal(
            vscode.Uri.parse(
      ...
      dcu.info({
        msg:
          "Actualizamos DCU UTILS a la version " +
          currentVersion +
          " para corregir algunos errores. \n ¿Quieres ver las mejoras del release " +
          lastRelease.V +
          "?",
        items: [CONST.SI, CONST.NO],
        callback: (response) => {
